  ...
  renderCards() {
    ...
    if (cardGrid) {
      ...
      if (!assets[this.level] || !assets[this.level].images) {
        console.error(`Assets for level "${this.level}" are missing or improperly loaded.`);
        return;
      }
      ...
      this.cards.forEach((value, index) => {
        ...
        const cardBackImage = assets[this.level].images.cardBack || 'assets/global/images/default/card-back.png';
